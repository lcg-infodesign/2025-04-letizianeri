...
function setup() {
  ...
  // Ciclo che estrae i dati dal CSV
  for (let i = 0; i < data.getRowCount(); i++) {
    ...
    let volcanoNumber = data.getString(i, "Volcano Number");

    if (!categories.includes(category)) categories.push(category);
    volcanoes.push({ name, country, elevation, category, status, eruption, volcanoNumber });
  }
